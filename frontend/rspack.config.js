...
export default (env) => {
    ...
    return {
        ...
        output: {
            ...
            // Always build into backend/www, in both watch (`npm start`)
            // and production (`npm run build`) modes, so the FastAPI
            // backend can serve it directly as static files.
            path: Path.resolve(__dirname, "../backend/www"),
        ...
        // No dev server: "npm start" just runs a watched build straight
        // into frontend/build. Use jarvis/jarvis.sh to start the backend
        // and open a browser.
        stats: {
            children: true,
            colors: true,
